...
import { OverlayContainer, SidebarContainer, CartSidebarHeader, SidebarContent, CartItem, CartList, CartSummary, ButtonCheckout, CloseSidebar } from '@/styles/components/Sidebar';
...
export function CartSidebar({ setIsOpen } : CartSideBarProps) {
    const { cart: { products, productIds, total }, removeProductToCart } = useContext(CartContext)
                                ...
                                {products.map(product => {
                                        return (
                                            <CartItem key={product.id}>
                                                <Image width={102} height={94} src={product.imageUrl} alt="" />
                                                <div>
                                                    <p>{product.name}</p>
                                                    <strong>{product.formattedPrice}</strong>

                                                    <button value={product.id} onClick={handleRemoveItemToCart}>Remover</button>
                                                </div>
                                            </CartItem>
                                        )
                                    })
